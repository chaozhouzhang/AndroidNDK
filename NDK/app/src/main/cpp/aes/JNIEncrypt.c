...
// 指定要注册的类，对应完整的java类名
#define JNI_REG_CLASS "android/stack/ndk/AESEncrypt"

const char *UN_SIGNATURE = "UN_SIGNATURE";
...
//隐藏字符表 并没有什么卵用 只是针对初阶hacker的一个小方案而已
char *getKey() {
    int n = 0;
...
int register_ndk_load(JNIEnv *env) {
    // 调用注册方法
    return registerNativeMethods(env, JNI_REG_CLASS,
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    //这是一种比较简单的防止被调试的方案
    //有更复杂更高明的方案，比如：不用这个ptrace而是每次执行加密解密签先去判断是否被trace，目前的版本不做更多的负载方案，想做可以fork之后，自己去做。
